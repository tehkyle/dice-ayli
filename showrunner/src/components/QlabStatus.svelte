...
{#if reachable && missingVars.length > 0}
  <div class="qlab-missing">
    <strong>Missing cues in QLab:</strong>
    {#each missingVars as v, i}
      <code>{v}</code>{#if i < missingVars.length - 1}, {/if}
    {/each}
  </div>
{/if}
